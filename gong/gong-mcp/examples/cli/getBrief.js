...
async function main() {
  ...
  if (callIdArgs.length === 0) {
    console.error('❌ Usage: node scripts/getBrief.js <CALL_ID> [<CALL_ID> ...]');
    process.exit(1);
  }
  ...
  try {
    ...
    if (!toolNames.includes('get_call_brief')) {
      throw new Error('get_call_brief tool not available');
    }

    console.error(`📝 Generating briefs for ${callIds.join(', ')}...`);
    ...
    const parsed = payload ? JSON.parse(payload) : { briefs: [] };

    for (const brief of parsed.briefs || []) {
      const id = brief?.callId ? String(brief.callId) : null;
      if (!id) continue;
      const outputPath = path.resolve(transcriptsDir, `${id}.brief.json`);
      fs.writeFileSync(outputPath, JSON.stringify(brief, null, 2), 'utf8');
      console.error(`💾 Saved brief to ${outputPath}`);
    }
  } catch (err) {
